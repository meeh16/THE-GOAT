    ...
    <div className="min-h-screen bg-slate-50/50 flex flex-col font-sans" id="app_root">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="font-display text-base font-extrabold text-gray-900 tracking-tight flex items-center gap-1.5">
                Sahur AI
                <span className="text-[10px] font-mono bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded-full font-bold">
                  v1.0
                </span>
              </h1>
          ...
          <nav className="flex gap-1.5 bg-gray-100 p-1 rounded-xl text-xs font-semibold">
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer ${
                activeTab === "evaluate" ? "bg-white text-indigo-600 shadow-xs" : "text-gray-500 hover:text-gray-700"
              }`}
              id="tab_evaluate"
            >
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer ${
                activeTab === "vault" ? "bg-white text-indigo-600 shadow-xs" : "text-gray-500 hover:text-gray-700"
              }`}
              id="tab_vault"
            >
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer ${
                activeTab === "chat" ? "bg-white text-indigo-600 shadow-xs" : "text-gray-500 hover:text-gray-700"
              }`}
              id="tab_chat"
            >
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 min-h-0 print:p-0 print:bg-white">
        ...
        {isPrintMode && reportResult && (
          <div className="bg-white p-8 max-w-3xl mx-auto border border-gray-300 rounded-none shadow-none space-y-8" id="print_passport_template">
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-5">
              <div>
                <span className="text-[10px] uppercase font-mono tracking-widest text-indigo-600 font-extrabold block">
                  Official Record • Indian Statutory Grounding
                </span>
                <h1 className="font-display text-2xl font-black text-slate-900 tracking-tight mt-1">
                  SAHUR CITIZEN COMPLIANCE PASSPORT
                </h1>
                <p className="text-xs text-slate-500 font-mono mt-0.5">
                  ID: PASSPORT_{activeAnalysis?.id?.split("_")[1]} • Generated on {new Date(activeAnalysis?.createdAt || "").toLocaleString()}
                </p>
              </div>
              <div className="text-right flex flex-col items-end gap-1 shrink-0 print:hidden">
                <button
                  onClick={handlePrint}
                  className="bg-indigo-600 text-white font-bold text-xs px-3 py-1.5 rounded-lg shadow-xs hover:bg-indigo-700 flex items-center gap-1 cursor-pointer"
                >
                  <Printer className="w-3.5 h-3.5" /> Print Passport
                </button>
                <button
                  onClick={() => setIsPrintMode(false)}
                  className="text-xs text-slate-500 hover:underline mt-1.5"
                >
                  Exit Print Preview
                </button>
              </div>
            ...
            {/* Print Core details */}
            <div className="grid grid-cols-3 gap-6 bg-slate-50 border border-slate-200 p-5 rounded-none text-xs">
              <div>
                <span className="block text-[10px] font-mono uppercase text-slate-400 font-bold mb-1">Evaluated Domain</span>
                <span className="font-bold text-slate-800 uppercase">{reportResult.domain}</span>
              </div>
              <div>
                <span className="block text-[10px] font-mono uppercase text-slate-400 font-bold mb-1">Compliance Safety Score</span>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {reportResult.rights_heatmap.map((r, i) => (
                  <div key={i} className="text-xs flex justify-between items-start border-b border-slate-100 pb-2">
                    <div className="pr-4">
                    ...
                    <span className={`font-mono text-[10px] uppercase font-bold shrink-0 ${
                      r.status === "violated" ? "text-red-600" : r.status === "compromised" ? "text-amber-600" : "text-green-600"
                    }`}>
                      {r.status}
                    </span>
                  </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2.5">
                {reportResult.applicable_laws.map((l, i) => (
                  <div key={i} className="text-xs p-3 bg-slate-50 border border-slate-100 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-6 pt-3 border-t border-slate-200">
              <div className="space-y-2">
                <h4 className="text-xs font-bold uppercase text-slate-900">Mandatory Evidence Gather Checklist</h4>
              ...
              <div className="space-y-2">
                ...
                <div className="text-xs text-slate-800">
                  <strong className="block font-bold">{reportResult.authority_recommendation.name}</strong>
                  <p className="text-slate-500 mt-0.5">{reportResult.authority_recommendation.description}</p>
                </div>
              </div>
            </div>

            {/* Footer Disclaimer */}
            <div className="border-t border-slate-200 pt-5 text-[10px] text-slate-400 text-center leading-relaxed">
              <strong>DISCLAIMER & RECORD ATTESTATION:</strong> Sahur AI is a legal knowledge system powered by RAG-retrieval. All citations relate to Indian Constitutional and statutory jurisprudence. This document does not constitute formal attorney representation or signed court advocacy counsel.
            </div>
          </div>
        )}
                  ...
                  <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm hover:shadow-md transition-all duration-300 space-y-6">
                    <div>
                      <h3 className="font-display text-lg font-bold text-gray-900 flex items-center gap-2">
                        <Scale className="w-5 h-5 text-indigo-600" /> Ground a New Case Study
                      </h3>
                      ...
                      <div className="flex flex-wrap gap-2.5">
                        {PRELOADED_SCENARIOS.map((preset, idx) => (
                          <button
                            ...
                            className="bg-slate-50 border border-slate-200 hover:border-indigo-500 hover:bg-indigo-50/30 text-xs font-semibold text-slate-700 px-4 py-2.5 rounded-full transition-all cursor-pointer shadow-2xs"
                    ...
                    <div className="space-y-5">
                      <div className="relative">
                        <textarea
                          ...
                          className="w-full px-5 py-4 border border-slate-200 rounded-2xl text-xs focus:outline-hidden focus:ring-2 focus:ring-indigo-500 font-sans bg-slate-50/30 text-slate-800"
                          id="scenario_input"
                        />
                      ...
                      <div className="flex justify-between items-center pt-2">
                        ...
                        <button
                          ...
                          className="bg-indigo-600 hover:bg-indigo-700 text-white disabled:bg-slate-100 disabled:text-slate-400 px-8 py-3.5 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all shadow-md hover:shadow-lg hover:scale-[1.02] cursor-pointer"
                          id="btn_run_analysis"
                        >
                          {isLoading ? (
                            <>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="space-y-3.5">
                              {reportResult.evidence_checklist.map((evidence, idx) => (
                                <div key={idx} className="flex items-start gap-3 text-xs text-slate-600">
                                  <input
                                    type="checkbox"
                                    className="mt-1 w-4.5 h-4.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                                  />
                          ...
                          <div className="bg-slate-900 rounded-[2rem] p-8 text-white shadow-xl space-y-6 relative overflow-hidden">
                            ...
                            <div className="absolute -right-12 -bottom-12 w-32 h-32 bg-indigo-500/20 rounded-full blur-3xl pointer-events-none" />

                            <h4 className="font-display text-base font-bold text-white relative z-10 flex items-center gap-2">
                              <Scale className="w-5 h-5 text-indigo-400" /> Escalation Authority
                            </h4>

                            <div className="p-4 bg-white/5 border border-white/10 rounded-xl space-y-1 relative z-10">
                              <span className="text-[9px] uppercase font-mono font-bold text-indigo-300 tracking-widest">Grievance Forum</span>
                            ...
                            <div className="space-y-4 relative z-10">
                              <span className="block text-[10px] font-bold text-indigo-300 uppercase tracking-widest font-mono">
                              ...
                              <div className="relative pl-3 space-y-4 before:content-[''] before:absolute before:left-1 before:top-1.5 before:bottom-1.5 before:w-[1px] before:bg-white/15">
                                {reportResult.authority_recommendation.stepByStepGrievance.map((step, idx) => (
                                  <div key={idx} className="relative text-xs text-slate-300">
                                    <span className="absolute -left-3.5 top-0.5 w-2.5 h-2.5 rounded-full bg-slate-800 border border-indigo-400 text-indigo-300 flex items-center justify-center text-[7px] font-bold">
                                      {idx + 1}
                                    </span>
                          ...
                          <div className="flex border-b border-slate-100 overflow-x-auto pb-2 shrink-0 gap-1.5 text-xs">
                            <button
                              ...
                              className={`px-4 py-2.5 font-bold rounded-lg whitespace-nowrap cursor-pointer transition-all ${
                                reportTab === "laws" ? "bg-indigo-50 text-indigo-600 shadow-2xs" : "text-slate-500 hover:text-slate-700"
                              }`}
                              id="inner_tab_laws"
                            >
                              Statutory Citations
                            ...
                            <button
                              ...
                              className={`px-4 py-2.5 font-bold rounded-lg whitespace-nowrap cursor-pointer transition-all ${
                                reportTab === "heatmap" ? "bg-indigo-50 text-indigo-600 shadow-2xs" : "text-slate-500 hover:text-slate-700"
                              }`}
                            ...
                            <button
                              ...
                              className={`px-4 py-2.5 font-bold rounded-lg whitespace-nowrap cursor-pointer transition-all ${
                                reportTab === "tree" ? "bg-indigo-50 text-indigo-600 shadow-2xs" : "text-slate-500 hover:text-slate-700"
                              }`}
                              id="inner_tab_tree"
                            >
                              Audit Timeline
                            ...
                            <button
                              ...
                              className={`px-4 py-2.5 font-bold rounded-lg whitespace-nowrap cursor-pointer transition-all ${
                                reportTab === "simulator" ? "bg-indigo-50 text-indigo-600 shadow-2xs" : "text-slate-500 hover:text-slate-700"
                              }`}
                              ...
                              <div className="space-y-5" id="statutes_panel">
                                <div>
                                  <h4 className="font-display text-sm font-bold text-slate-800 flex items-center gap-1.5">
                                    <BookOpen className="w-4.5 h-4.5 text-indigo-600" /> Applicable Statutes Citation
                                  </h4>
                                  ...
                                  {reportResult.applicable_laws.map((law, idx) => (
                                    <div key={idx} className="p-5 border border-slate-100 rounded-2xl space-y-3 bg-slate-50/50">
                                      <div className="flex justify-between items-start gap-4">
                                        ...
                                        <span className="text-[9px] font-mono bg-indigo-50 border border-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded font-bold uppercase shrink-0">
                                          Grounding Source
                                        </span>
                                      </div>
